...
// 安装包目录
defined('INSTALL_PATH') or define('INSTALL_PATH', APP_PATH . 'admin/command/Install/');
...
if (is_file($lockFile)) {
    ...
    $errInfo = '当前权限不足，无法写入配置文件config/database.php<br><a href="javascript:;" target="_blank">点击查看解决办法</a>';
} else {
    $dirArr = [];
    foreach ($checkDirs as $k => $v) {
        if (!is_dir(ROOT_PATH . $v)) {
            $errInfo = '当前代码仅包含核心代码，请前往官网下载完整包或资源包覆盖后再尝试安装，<a href="javascript:;" target="_blank">立即前往下载</a>';
            break;
        }
    }
}
...
if (!$errInfo && isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $mysqlDatabase = isset($_POST['mysqlDatabase']) ? $_POST['mysqlDatabase'] : 'jzadmin';
    ...
    try {
        //检测能否读取安装文件
        $sql = @file_get_contents(INSTALL_PATH . 'jzadmin.sql');
        if (!$sql) {
            throw new Exception("无法读取application/admin/command/Install/jzadmin.sql文件，请检查是否有读权限");
        }
        $sql = str_replace("`jz_", "`{$mysqlPrefix}", $sql);
        ...
        $newSalt = substr(md5(uniqid(true)), 0, 4);
...
<div class="container">
    ...
    <div>
        ...
        <p>若你在安装中遇到麻烦可联系260591808@qq.com</p>
        ...
        <form method="post">
            ...
            <div class="form-group">
                ...
                <div class="form-field">
                    ...
                    <input type="text" name="mysqlPrefix" value="jz_">
                </div>
            </div>
                    ...
                    $.post('', $(this).serialize())
                        .done(function (ret) {
                            if (ret === 'success') {
                                ...
                                $('<a class="btn" href="./">访问首页</a> <a class="btn" href="/admin/login/index" style="background:#18bc9c">访问后台</a>').insertAfter($button);
                                $button.remove();
                                localStorage.setItem("step", "installed");
                            } else {
                                $('#error').show().text(ret);
