...
class Auth {
    ...
    public function getGroups($uid) {
        ...
        $user_groups = Db::name($this->config['auth_group_access'])
            ->alias('a')
            ->join('__' . strtoupper($this->config['auth_group']) . '__ g', 'a.group_id = g.id', 'LEFT')
            ->field('a.uid,a.group_id,g.id,g.title,g.rules')
    ...
    public function getRuleList($uid) {
        ...
        //读取用户组所有权限规则
        $this->rules = Db::name($this->config['auth_rule'])->where($where)->field('id,pid,condition,name,title,is_menu')->select();
